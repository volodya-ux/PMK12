...
public:
    ...
    map<Owner, Phone> searchPhonesByOwner(const string& name) const { //пошук номерів телефонів заданого власника та їх переміщення у інший контейнер
    ...
    map<Owner, Phone> searchPhonesByAddress(const string& address) const { // пошук власника за адресою
        map<Owner, Phone> result;
        for (const auto& entry : phones) {
            const Owner& owner = entry.first;
            const Phone& phone = entry.second;
    ...
    map<Owner, Phone> searchPhonesByManufacturerAndPrice(const string& manufacturer, double price) const { // пошук всіх власників телефонів заданого виробника ціна яких перевищує задану
        map<Owner, Phone> result;
        for (const auto& entry : phones) {
            const Owner& owner = entry.first;
            const Phone& phone = entry.second;
    ...
    Owner findOwnerByPhone(const string& phone) const {  //пошук власника за номером
