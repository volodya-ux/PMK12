...
public:
    ...
    friend ostream& operator<<(ostream& os, const Triangle& triangle) {
        os << " ѕериметр трикутника: " << triangle.perimetr << endl;
        return os;
    }
...
int main()
{
    ...
    cout << "¬вед≥ть м≥н≥мальний периметр: ";
    cin >> MinPerimetr;
    cout << "¬вед≥ть максимальний периметр: ";
    cin >> MaxPerimetr;
    for (int i = 0; i < count; i++) {
        ...
        {
            outfile << tri[i].name << " сторони трикутника: " << tri[i].side[0] << " " << tri[i].side[1] << " " << tri[i].side[2] << " периметр трикутника: " << tri[i].perimetr << " площа трикутника(з точн≥стю до дес€тих): " << tri[i].area << endl;
